...
static int is_inside(const char* src, const char* trgt)
{
    ...
    if (realpath(src, r_src) == NULL || realpath(trgt, r_trgt) == NULL)
        return 0;
    size_t len = strlen(r_src);
    if (strncmp(r_src, r_trgt, len) == 0)
    {
        if (r_trgt[len] == '/' || r_trgt[len] == '\0')
            return 1;
    }
    return 0;
}
...
void start_all_backups(cmd_t* cmd)
{
    ...
    if (realpath(cmd->src, abs_src) == NULL)
    {
    ...
    for (int i = 0; i < cmd->target_count; i++)
    {
        ...
        if (realpath(cmd->trgt[i], abs_trgt) == NULL)
            continue;
...
void stop_specific_backups(cmd_t* cmd)
{
    ...
    // src do tablicy
    if (realpath(cmd->src, abs_src) == NULL)
        strncpy(abs_src, cmd->src, PATH_MAX - 1);

    for (int j = 0; j < 64; j++)
    {
        if (!tasks[j].active)
            continue;
        if (strcmp(tasks[j].src, abs_src) != 0)
            continue;
        ...
        else
        {
            for (int i = 0; i < cmd->target_count; i++)
            {
                ...
                if (realpath(cmd->trgt[i], abs_trgt) == NULL)
                    strncpy(abs_trgt, cmd->trgt[i], PATH_MAX - 1);
                if (strcmp(tasks[j].trgt, abs_trgt) == 0)
                {
                    should_kill = 1;
                    break;
                }
