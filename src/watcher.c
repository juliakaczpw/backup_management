...
struct WatchMap { struct Watch watch_map[MAX_WATCHES]; int watch_count; };

static void add_to_map(struct WatchMap *map, int wd, const char *path) {
    if (map->watch_count >= MAX_WATCHES) return;
...
static struct Watch *find_watch(struct WatchMap *map, int wd) {
    for (int i = 0; i < map->watch_count; i++) if (map->watch_map[i].wd == wd) return &map->watch_map[i];
    return NULL;
}
...
static void add_watch_recursive(int notify_fd, struct WatchMap *map, const char *base_path) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        struct stat st;
        if (lstat(full_path, &st) == 0 && S_ISDIR(st.st_mode)) add_watch_recursive(notify_fd, map, full_path);
    }
    closedir(dir);
}
...
void start_watcher(const char *source_path, const char *target_root) {
    ...
    if (realpath(source_path, abs_source) == NULL) exit(1);

    int last_slash = -1;
    for (int k = 0; abs_source[k] != '\0'; k++) if (abs_source[k] == '/') last_slash = k;
            ...
            if (watch) {
                ...
                if (event->len > 0) {
                    ...
                    if (event->mask & (IN_CREATE | IN_MOVED_TO)) {
                        ...
                        struct stat st;
                        if (lstat(s_path, &st) == 0 && S_ISDIR(st.st_mode)) add_watch_recursive(notify_fd, &map, s_path);
                    } else if (event->mask & IN_MODIFY) {
                        struct stat st;
                        if (lstat(s_path, &st) == 0 && S_ISREG(st.st_mode)) copy_file(s_path, t_path);
                    } else if (event->mask & (IN_DELETE | IN_MOVED_FROM)) {
                        remove_recursive(t_path);
                    }
                }
            }
            if (event->mask & IN_IGNORED) remove_from_map(&map, event->wd);
